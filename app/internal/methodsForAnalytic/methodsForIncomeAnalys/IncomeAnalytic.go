...
type IncomeAnalyticHandlers interface {
	...
	IncomeWeekAnalytic(update tgbotapi.Update) ([]models.Transactions, error)
	IncomeMonthAnalytic(update tgbotapi.Update) ([]models.Transactions, error)
}
...
//func (an *AnalyticHandler) IncomeMonthAnalytic(update tgbotapi.Update) ([]models.Transactions, error) {
//	return nil
//}

func GenerateMonthlyIncomeReport(transactions []models.Transactions) string {
	if len(transactions) == 0 {
		return "📈 Сегодня у вас не было доходов."
	}
